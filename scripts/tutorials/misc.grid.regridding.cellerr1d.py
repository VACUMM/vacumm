"""Interpolate 1D data with cell averaging taking error into account"""
...
lags = N.arange(1, 6)
for lag in lags:
    els.append(N.sqrt(((sp[lag:]-sp[:-lag]).asma()**2).mean()))
els = N.array(els)
...
curve2(sph, '-r', err=speh.asma()/2., linewidth=1.5, show=False, key=2,
    label='Interpolated', legend=True, tight_layout=True,
